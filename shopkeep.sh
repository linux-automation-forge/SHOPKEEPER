...
cmd_add() {
    local name="" price="" qty="" bc="" thr="" desc="" ptype="" image="" expiry="" hsn=""
    while [[ $# -gt 0 ]]; do
        case "$1" in
...
cmd_doctor() {
    ...
    printf "  QR codes: %s (zint -b 58; SVG fallback)   EAN-13 labels: %s (zint -b 13 for 12/13-digit barcodes)\n" \
        "$(command -v zint >/dev/null && echo PNG || echo SVG-fallback)" \
        "$(command -v zint >/dev/null && echo PNG || echo SVG-fallback)"
    local gstin; gstin=$(conf gstin "")
...
menu_add_product() {
    printf '%s' "$(t p_name): "; local name=""; read -r name || true
    ...
    printf '%s' "$(t p_threshold) (blank=default): "; local thr=""; read -r thr || true
    printf '%s' "$(t p_type) (blank=skip): "; local ptype=""; read -r ptype || true
    printf '%s' "$(t p_expiry) (blank=none): "; local expiry=""; read -r expiry || true
    printf '%s' "$(t p_hsn) (blank=none): "; local hsn=""; read -r hsn || true
    cmd_add --name "$name" --price "$price" --qty "$qty" \
        ${bc:+--barcode "$bc"} ${thr:+--threshold "$thr"} \
        ${ptype:+--type "$ptype"} ${expiry:+--expiry "$expiry"} ${hsn:+--hsn "$hsn"}
}
